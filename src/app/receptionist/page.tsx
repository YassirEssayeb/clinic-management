...
import { getDashboardStats as getRecStats } from "@/actions/receptionist.actions"
...
export default function ReceptionistDashboardPage() {
  ...
  useEffect(() => {
    getRecStats().then((data) => { setStats(data); setLoading(false) }).catch(() => setLoading(false))
  }, [])
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[...Array(4)].map((_, i) => <div key={i} className="bg-white border border-[#c3c6d5] rounded-xl p-6 h-auto min-h-[8rem] animate-pulse" />)}
        </div>
      </div>
    )
  }
  ...
  const metricCards = [
    ...
    { label: "Pending Appointments", value: stats?.pendingAppointments ?? 0, icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
    { label: "Total Patients", value: stats?.totalPatients ?? 0, icon: Users, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Total Doctors", value: stats?.totalDoctors ?? 0, icon: UserCog, color: "text-violet-600", bg: "bg-violet-50" },
  ]
