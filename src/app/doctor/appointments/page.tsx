...
import { getMyAppointments, updateAppointmentStatus } from "@/actions/doctor.actions"
...
export default function DoctorAppointmentsPage() {
  ...
  const fetchAppointments = useCallback(async () => {
    setLoading(true)
    try {
      const res = await getMyAppointments({ search, status: statusFilter, page, pageSize: 10 })
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold text-[#111c2d]">My Appointments</h1>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[#f0f3ff]">
              ...
              <TableHead className="text-xs font-semibold text-[#434653] uppercase tracking-wider">Reason</TableHead>
                ...
                <TableRow key={a.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-sm font-medium text-[#111c2d]">{a.patient?.user?.name}</p>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-sm text-[#434653]">{a.reason ?? "—"}</TableCell>
